...
Le projet de mandat est disponible [ici](https://cnig.gouv.fr/IMG/pdf/4.6_mandat_de_gt-pdipr.pdf).
...
Le standard PDIPR est actuellement en phase de : **Réalisation du standard**

Le projet de standard PDIPR, puis le standard une fois validé par le CNIG, évoluera en fonction des évolutions réglementaires et de l'expression des besoins de la communauté des utilisateurs.

Les ressources associées et les bases de données correspondantes seront actualisées conformément au standard CNIG PDIPR. Les mises à jour de base de données sont effectuées en modifiant le cas échéant les données qui y figurent déjà.
